...
export default function TrasladoCheckoutPage() {
  ...
  const handleBook = async (e: React.FormEvent) => {
    ...
    try {
      await authFetch('/bookings', {
        method: 'POST',
        body: JSON.stringify({ type: 'taxi', from, to, date, pickupTime: time, contact }),
      });
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', padding: '32px 16px' }}>
      <Link href="/traslados" className="text-sm text-primary inline-flex items-center gap-2 mb-4 hover:underline">
        <i className="fa fa-arrow-left" />Volver
      </Link>
      <h1 className="text-2xl font-bold text-gray-800 font-poppins mb-4">Checkout — Traslado</h1>
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6 text-sm text-gray-600">
        <p><strong className="text-primary">{from} → {to}</strong>{date && ` · ${date}`}{time && ` · ${time}`}</p>
      </div>
